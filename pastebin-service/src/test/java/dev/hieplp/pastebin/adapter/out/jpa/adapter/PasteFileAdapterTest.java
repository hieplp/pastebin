...
class PasteFileAdapterTest {
    ...
    @Mock
    private dev.hieplp.pastebin.adapter.out.jpa.mapper.VoMapper voMapper;
    ...
    void deleteByPasteIds_whenEmptyOrNull_doesNothing() {
        ...
        verify(pasteFileRepo, never()).deleteAllInBatch(any());
    }

    @Test
    void deleteByPasteIds_whenFilesFound_deletesRecords() {
        var file1 = new PasteFileEntity();
        file1.setFileId("f-1");
        file1.setStorageKey("uploads/p-1/f-1.txt");

        var file2 = new PasteFileEntity();
        file2.setFileId("f-2");
        ...
        verify(pasteFileRepo).deleteAllInBatch(List.of(file1, file2));
    }
